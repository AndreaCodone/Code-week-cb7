...
function separateAppointmentsByType(appointments) {
  ...
  appointments.forEach((appointment) => {
    ...
    separatedAppointments[typeName] = separatedAppointments[typeName] || [];
...
function createAppointmentElement(appointment) {
  ...
  function completeAppointmentHandler() {
    appointmentContainer.classList.toggle("completed");
    console.log(`Appuntamento completato con ID: ${appointment.id}`);
  }
...
function handleScrollSpy() {
  const scrollspyNav = qS(".custom-nav");
  const sections = qSA(".scrollspy-section");
...
function saveDeletedAppointment(appointment) {
  const deletedAppointments =
    JSON.parse(localStorage.getItem("deletedAppointments")) || [];
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const navItems = qSA(".custom-nav li");
  ...
  window.addEventListener("scroll", () => {
    scrollToTopButton.classList.toggle("show", window.pageYOffset > 100);
    handleScrollSpy();
  });
